...
define([
    ...
    GraphDrawer.prototype.draw_minimap = function (svg, sizes, data, stack) {
        ...
        else if (gui_manager.graph_type == "heat") {
            var x_width = sizes.width - (sizes.margin.left + sizes.margin.right);
            var y_width = sizes.height_mini - (sizes.margin.top + sizes.margin.bottom);
            var margin_left = sizes.margin.left + sizes.margin.right * 2;
            var margin_top = sizes.margin.top;
        ...
        function brushed() {
            ...
            if (s != null && s.length == 2) {
                ...
                var s_1 = drawer.events.findIndex(function (e) {
                    return moment(e).isSameOrAfter(moment(raw_start));
                });
                var e_1 = drawer.events.findIndex(function (e) {
                    return moment(e).isSameOrAfter(moment(raw_end));
                });
                if (s_1 == e_1) {
                    if (s_1 == 0)
                        e_1++;
                    else if (e_1 == drawer.events.length - 1)
                        s_1--;
                    else {
        ...
        function draw_stream(data, stack) {
            ...
            drawer.mini_x.domain(d3.extent(data, function (d) {
                return d.date;
            }));
            var area = d3.area()
                .x(function (d, i) {
                    return drawer.mini_x(d.data.date);
                })
                .y0(function (d) {
                    return drawer.mini_y(d[0]);
                })
                .y1(function (d) {
                    return drawer.mini_y(d[1]);
                });
    ...
    GraphDrawer.prototype.draw_streamgraph = function (current_parsed, graph_type, tsv_incoming_data, keys_order, preserve_color_map, global_visibility, targets, query_id, bgplay_callback, events_limit, events_range, redraw_minimap) {
        ...
        var g = this.main_svg.append("g")
            ...
            .on('mouseout', function () {
                if (!drawer.gui_manager.steps) mouseout()
            })
            .on('mouseover', function () {
                if (!drawer.gui_manager.steps) mouseover()
            })
            .on('click', function () {
        ...
        if (this.events_range) {
            data = data.filter(function (e) {
                return moment(e.date).isSameOrAfter(drawer.events_range[0]) && moment(e.date).isSameOrBefore(drawer.events_range[1]);
            })
        }
        ...
        layer.append("path")
            ...
            .on('mousemove', function (d) {
                if (!drawer.gui_manager.steps) mousemove(d, d3.mouse(this))
            });
        ...
        layer.filter(function (d) {
            ...
            .attr("y", function (d) {
                return drawer.y((d[d.length - 1][0] + d[d.length - 1][1]) / 2);
            })
            .attr("dy", ".35em")
            ...
            .style("z-index", "999")
            .style("fill", function (d) {
                return drawer.color_manager.furthestLabelColor(drawer.z(d.key))
            })
            .text(function (d) {
                return d.key;
            });
    ...
    GraphDrawer.prototype.common_for_streamgraph = function (tsv_data, keys_order, events_limit, visibility, preserve_color_map, query_id) {
        ...
        if (!preserve_color_map || this.current_query_id != query_id || this.colors.length != this.keys.length) {
            this.colors = this.color_manager.d_sorteds.map(function (c) {
                return c.lab.rgb()
    ...
    GraphDrawer.prototype.draw_heatmap = function (current_parsed, tsv_incoming_data, stream_tsv, keys_order, preserve_color_map, global_visibility, targets, query_id, bgplay_callback, level, ip_version, prepending, collapse_rrc, collapse_events, events_labels, rrc_labels, timemap, events_range, redraw_minimap) {
        ...
        var RRCLabels = g
            ...
            .attr("x", 0)
            .attr("y", function (d, i) {
                return (i * (gridSize_y + drawer.sizes.def_cell_margins.y));
            })
        ...
        var EventsLabels = g
            ...
            .attr("x", 0)
            .attr("y", function (d, i) {
                return (i * (gridSize_x + drawer.sizes.def_cell_margins.x));
            })
        ...
        areas.enter().append("rect")
            ...
            .style("fill", function (d) {
                return (d.asn && d.asn != null) ? drawer.z(d.asn) : "#ffffff";
            })
            .style("stroke", "black")
        ...
        if (!collapse_rrc) {
            ...
            Flags
                ...
                .attr("x", 20)
                .attr("y", function (d, i) {
                    return (i * (gridSize_y + drawer.sizes.def_cell_margins.y) - 7);
                });
        }
        areas.exit().remove();
    ...
    GraphDrawer.prototype.shuffle_color_map = function (graph_type) {
        ...
        if (graph_type == "stream") {
            this.colors = random_sort(this.color_manager.d_sorteds.map(function (c) {
                return c.lab.rgb()
            }), this.keys.length);
            this.z = d3.scaleOrdinal(this.colors.slice(0).reverse());
            this.z.domain(this.keys);
