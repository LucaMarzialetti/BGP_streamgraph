...
GraphDrawer.prototype.draw_heatmap = function(current_parsed, tsv_incoming_data, stream_tsv, keys_order, preserve_color_map, global_visibility, targets, query_id, bgplay_callback, level, ip_version, prepending, collapse_cp, collapse_events, events_labels, cp_labels, timemap, events_range, redraw_minimap){
	...
	if(collapse_cp){
		...
		data=data.filter(function (e){var k = false; for(var i in cp_to_filter) k=k||cp_to_filter[i].indexOf(e.cp)==0; return k;});
		this.cp_set=cp_to_filter.map(function (e) {return e[0];});
	}
	data.columns=tsv_data.columns;
	...
	if(keys_order){
		...
		if(collapse_cp)
			this.keys = keys_order.filter(function(e){return drawer.cp_set.indexOf(e)>=0;}); //QUI
		else 
 ...
 //    }
	...
	areas.enter().append("rect")
		...
		.attr("y", function(d) {return (drawer.keys.indexOf(d.cp) * (gridSize_y+drawer.sizes.def_cell_margins.y)); })
		.attr("class", function(d){return "area area"+d.cp.replace(/[\.:]/g,"-")+" area"+d.date.replace(/:/g,"-")+" area"+d.asn})
